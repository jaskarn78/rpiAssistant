(function() {
	...
	function setValues(){
		var time = (moment().format(formatTimeUTC)).split(':');
			var hour = time[0];
			if(parseInt(hour) < 12)
				$('#name').html("Good Morning\nJaskarn");
			else if(parseInt(hour) == 12 || parseInt(hour) < 16)
				$("#name").html("Good Afternoon\nJaskarn");
			else $("#name").html("Good Evening\nJaskarn");
	}
	...
	$(window).load(function() {
		...
		setInterval(function(){
			...
			if ($('#date').length) {
				...
				//var end = moment("2018-04-13", "YYYY-MM-DD");
